 ...
 * Desc:
 * 
 * Source: https://guidedhacking.com/threads/createremotethread-equivalent-in-kernel-mode.11213/
 */
uintptr_t GetModuleBaseAddress( DWORD proc_id, const char* mod_name )
{
    ...
    if( hSnap != INVALID_HANDLE_VALUE )
    {
        ...
        if( Module32First( hSnap, &mod_entry ) )
        {
            do
            {
                std::wstring wstrModName( sizeof( mod_name ), L'#' );
                mbstowcs( &wstrModName[0], mod_name, sizeof( mod_name ) );

                //if (!_stricmp(modEntry.szModule, modName))
                if( std::wstring( mod_entry.szModule ) == wstrModName )
                {
                    modBaseAddr = (uintptr_t) mod_entry.modBaseAddr;
                    break;
                }
